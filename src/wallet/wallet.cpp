...
// pozoqo: optional setting to unlock wallet for block minting only;
...
{
    // pozoqo: we still have the timestamp, so use it to avoid confusion
    if (tx->nTime)
        return tx->nTime;
...
bool CWalletTx::SubmitMemoryPoolAndRelay(std::string& err_string, bool relay)
{
    ...
    bool ret = pwallet->chain().broadcastTransaction(tx, relay, err_string);
    fInMempool |= ret;
    return ret;
}
...
void CWallet::AvailableCoins(interfaces::Chain::Lock& locked_chain, std::vector<COutput>& vCoins, bool fOnlySafe, const CCoinControl* coinControl, const uint32_t nSpendTime, const CAmount& nMinimumAmount, const CAmount& nMaximumAmount, const CAmount& nMinimumSumAmount, const uint64_t nMaximumCount) const
{
    AssertLockHeld(cs_wallet);
    ...
    for (const auto& entry : mapWallet)
    {
        ...
        if (nSpendTime > 0 && wtx.tx->nTime > nSpendTime)
            continue;  // pozoqo: timestamp must not exceed spend time
                         ...
                         int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    int nBytes;
    {
        ...
        LOCK(cs_wallet);
        {
            ...
            AvailableCoins(*locked_chain, vAvailableCoins, true, &coin_control, txNew.nTime, 1, MAX_MONEY, MAX_MONEY, 0);
...
//                }
                ...
                // pozoqo: sub-cent change is moved to fee
...
// pozoqo: create coin stake transaction
