 ...
 */
static const char* BITCOIN_PID_FILENAME = "pozoqod.pid";
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/pozoqo/pozoqo>";
...
/** Sanity checks
 *  Ensure that Pozoqo is running in a usable environment with all
...
{
    // Make sure only a single Pozoqo process is using the data directory.
...
bool AppInitSanityChecks()
{
    ...
    // pozoqo: init hash seed
    pozoqoRandseed = GetRand(1 << 30);
...
bool AppInitMain(NodeContext& node)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if pozoqo is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if pozoqo is started while in a temporary directory.\n",
