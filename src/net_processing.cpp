...
namespace {
    ...
    /** pozoqo: blocks that are waiting to be processed, the key points to previous CBlockIndex entry */
    struct WaitElement {
...
void static ProcessGetBlockData(CNode* pfrom, const CChainParams& chainparams, const CInv& inv, CConnman* connman)
{
    ...
    if (send && (pindex->nStatus & BLOCK_HAVE_DATA))
    {
        ...
        if (inv.hash == pfrom->hashContinue)
        {
            ...
            // pozoqo: send latest proof-of-work block to allow the
...
void static ProcessOrphanTx(CConnman* connman, CTxMemPool& mempool, std::set<uint256>& orphan_work_set, std::list<CTransactionRef>& removed_txn) EXCLUSIVE_LOCKS_REQUIRED(cs_main, g_cs_orphans)
{
    ...
    while (!done && !orphan_work_set.empty()) {
        ...
        if (AcceptToMemoryPool(mempool, orphan_state, porphanTx, false /* bypass_limits */, false /* test_accept */)) {
...
bool ProcessMessage(CNode* pfrom, const std::string& msg_type, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CTxMemPool& mempool, CConnman* connman, BanMan* banman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    // pozoqo: set/unset network serialization mode for new clients
    ...
    if (msg_type == NetMsgType::GETBLOCKS) {
        ...
        for (; pindex; pindex = ::ChainActive().Next(pindex))
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // pozoqo: tell downloading node about the latest block if it's
    ...
    if (msg_type == NetMsgType::TX) {
        ...
        if (!AlreadyHave(inv, mempool) &&
            AcceptToMemoryPool(mempool, state, ptx, false /* bypass_limits */, false /* test_accept */)) {
            mempool.check(&::ChainstateActive().CoinsTip());
    ...
    if (msg_type == NetMsgType::HEADERS)
    {
        ...
        for (unsigned int n = 0; n < nCount; n++) {
            ...
            // pozoqo: quick check to see if we should ban peers for PoS spam
    ...
    if (msg_type == NetMsgType::BLOCK)
    {
...

        {
            ...
            // pozoqo: store in memory until we can connect it to some chain
        ...
        // pozoqo: accept as many blocks as we possibly can from mapBlocksWait
        while (fContinue) {
            fContinue = false;
            ...
            {
            LOCK(cs_main);
            // pozoqo: try to select next block in a constant time
