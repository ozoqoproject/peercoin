...
// pozoqo: if pwallet != NULL it will attempt to create coinstake
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool* pfPoSCancel)
{
    ...
    // pozoqo: if coinstake available add coinstake tx
    ...
    if (pwallet)  // attemp to find a coinstake
    {
        *pfPoSCancel = true;
        ...
        if (*pfPoSCancel)
            return nullptr; // pozoqo: there is no point to continue if we failed to create coinstake
...
static bool ProcessBlockFound(const CBlock* pblock, const CChainParams& chainparams)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != ::ChainActive().Tip()->GetBlockHash())
            return error("PozoqoMiner: generated block is stale");
    }
...
void PoSMiner(std::shared_ptr<CWallet> pwallet, CConnman* connman, CTxMemPool* mempool)
{
    ...
    util::ThreadRename("pozoqo-stake-minter");
    ...
    try {
        ...
        while (true) {
...

            {
                ...
                {
                    LogPrintf("PozoqoMiner runtime error: %s\n", e.what());
                    continue;
                }
            }
            ...
            if (!pblocktemplate.get())
            {
                ...
                uiInterface.NotifyAlertChanged(uint256(), CT_UPDATED);
                LogPrintf("Error in PozoqoMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                if (!connman->interruptNet.sleep_for(std::chrono::seconds(10)))
                   return;
            ...
            // pozoqo: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
                {
                    LOCK2(cs_main, pwallet->cs_wallet);
    ...
    {
        LogPrintf("PozoqoMiner terminated\n");
        return;
    }
    catch (const std::runtime_error &e)
    {
...
{
    // pozoqo: mint proof-of-stake blocks in the background
